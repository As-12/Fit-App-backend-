...
class ProgressesTest(Resource):
    ...
    def get(self, payload, user_id):
        ...
        and return all progress"""

        logger.info(f"GET request to generate test progress list "
                    f"for {user_id} "
                    f"from {request.remote_addr}")
        user = User.query.get(user_id)
        if user is None:
            code = 404
            message = f"Cannot generated progress for {user_id}. " \
            ...
            progress_list = Progress.query \
                .filter(Progress.user_id == user_id) \
                .order_by(desc(Progress.track_date)).all()
            response = {
                "user_id": user_id,
                "progresses": progress_list,
                "count": len(progress_list)
...
class Progresses(Resource):
    ...
    def get(self, payload, user_id):
        ...
        logger.info(f"GET request to progress list of {user_id} "
                    f"from {request.remote_addr}")
